...
<html lang="en">
  ...
  <meta charset="UTF-8">
    ...
    <?php
        ...
        $phongBan = $_GET['tenPB'];
        $sql = "SELECT * FROM `phongban` WHERE `tenPB` = ?";
        $stm = $conn->prepare($sql);
        
        $stm->bind_param("s", $phongBan);
        if(!$stm->execute()) {
            die ("Lỗi" . $stm->error);
        }
        $result = $stm->get_result();
    ...
    <div class="container-fluid">
      ...
      <div class="row">
        <div class="col-md-8 col-lg-5 my-5 mx-2 mx-sm-auto border rounded px-3 py-3 bg-white">
          <h5 class="text-center mb-1">Sửa thông tin phòng ban</h5>
          <form onsubmit="return validateDepartment()"; method="post" action="./process/update-department.php">
